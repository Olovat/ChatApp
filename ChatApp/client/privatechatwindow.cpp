...
{
    // ИСПРАВЛЕНИЕ: Убираем метку статуса полностью
...
{
    // ИСПРАВЛЕНИЕ: Убираем отображение статуса
...
void PrivateChatWindow::addMessageToChat(const QString &sender, const QString &content, const QString &timestamp)
{
    ...
    // ИСПРАВЛЕНИЕ: Правильное определение отправителя
    if (sender == m_peerUsername) {
        ...
        // Сообщение от текущего пользователя (отправителя)
    ...
    qDebug() << "Added message to chat: sender=" << sender 
             << ", peer=" << m_peerUsername 
             << ", content=" << content;
}
...
void PrivateChatWindow::receiveMessage(const QString &sender, const QString &message, const QString &timestamp)
{
    ...
    // ИСПРАВЛЕНИЕ: Если окно видимо, сразу помечаем как прочитанные
    if (this->isVisible()) {
        emit markAsReadRequested(m_peerUsername);
    }
    
    // Активируем окно только если оно не активно
...
void PrivateChatWindow::showEvent(QShowEvent *event)
{
    ...
    // ИСПРАВЛЕНИЕ: Сразу помечаем как прочитанные при показе окна
    emit markAsReadRequested(m_peerUsername);
    
    // ИСПРАВЛЕНИЕ: Активируем окно и устанавливаем фокус
    this->activateWindow();
    this->raise();
    this->setFocus();
    
    // Запрашиваем историю при показе окна
