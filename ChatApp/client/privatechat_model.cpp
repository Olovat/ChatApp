...
void PrivateChatModel::addMessage(const PrivateMessage &message)
{
    ...
    // ИСПРАВЛЕНИЕ: НЕ увеличиваем счетчик, если сообщение уже помечено как прочитанное
    if (message.sender != m_currentUser && !message.isRead) {
        m_unreadCount++;
        emit unreadCountChanged(m_unreadCount);
        qDebug() << "Increased unread count to" << m_unreadCount << "for sender" << message.sender 
                 << ", isRead=" << message.isRead;
    } else {
        qDebug() << "NOT increasing unread count: sender=" << message.sender 
...
{
    // ИСПРАВЛЕНИЕ: Полная замена истории вместо добавления
    m_messages.clear(); // Очищаем существующие сообщения
    m_messages = messages;
    updateUnreadCount();
    emit messagesUpdated();
}
